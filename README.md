...
```bash
# Build image
docker build -t loomiq:latest .

# Run with environment variables
docker run -d \
  --name loomiq-orchestrator \
...
# View logs
docker logs -f loomiq-orchestrator
```
...
services:
  postgres:
    ...
    ports:
      - "5432:5432"
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U loomiq_user"]
      interval: 10s
      timeout: 5s
      retries: 5
  ...
  loomiq:
    ...
    environment:
      ...
      PORT: 3000
      WEBSOCKET_PORT: 4000
      DATABASE_URL: postgresql://loomiq_user:${DB_PASSWORD}@postgres:5432/loomiq
...
server {
    ...
    # WebSocket endpoints (MCP)
    location /socket.io/ {
        proxy_pass http://loomiq_ws;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host $host;
